...
if [[ $CKAN__PLUGINS == *"datapusher"* ]]; then
    ...
    ckan config-tool $CKAN_INI "ckanext.spatial.common_map.attribution = Map tiles by <a href=\"http://www.gsi.go.jp\" target=\"_blank\">GSI</a>"
    ...
    ckan config-tool $CKAN_INI "ckanext.sip4d.pass_phrase = ${JWT_SECRET}"
else
   echo "Not configuring SIP4D-CKAN"
fi
